 ...
 */
export class PushPayment extends NequiRequest {
  async createPayment(unregisteredPaymentRQ: CreatePaymentPushBody) {
    const req = await this.post<CreatePaymentPushResponse>(`${URLS.BASE_PATH}${ENDPOINTS.PAYMENT_PUSH.UNREGISTERED}`, {
      body: JSON.stringify({
        RequestMessage: {
          RequestHeader: {
  ...
  async cancel(cancelUnregisteredPaymentRQ: CancelUnregisteredPaymentBody) {
    const req = await this.post<CancelUnregisteredPaymentResponse>(
  ...
  async getStatus(getStatusPaymentRQ: GetStatusPaymentBody) {
    const req = await this.post<GetStatusPaymentResponse>(`${URLS.BASE_PATH}${ENDPOINTS.PAYMENT_PUSH.STATUS}`, {
      body: JSON.stringify({
        RequestMessage: {
          RequestHeader: {
            Channel: CHANNELS.PAYMENT_PUSH,
  ...
  async revertTransaction(reversionRQ: RevertTransactionBody) {
    const req = await this.post<RevertTransactionResponse>(`${URLS.BASE_PATH}${ENDPOINTS.PAYMENT_PUSH.REVERT}`, {
      body: JSON.stringify({
        RequestMessage: {
          RequestHeader: {
            Channel: "PNP04-C001",
